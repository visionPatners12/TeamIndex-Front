  ...
  const FEATURES = [
    {
      ...
      title: t('marketplace.instantTradingTitle'),
      desc: t('marketplace.instantTradingDesc'),
    },
    {
      icon: (
        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      ...
      title: t('marketplace.crossIndexTitle'),
      desc: t('marketplace.crossIndexDesc'),
    },
    {
      icon: (
        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      ...
      title: t('marketplace.portfolioTitle'),
      desc: t('marketplace.portfolioDesc'),
    },
  ];
    ...
    <section className="w-full py-24 px-4 sm:px-10 lg:px-30 bg-[#0D0A06]">
      <div
        ...
        style={{
          padding: "2px",
          borderRadius: "30px",
          background: "linear-gradient(135deg, #FEB413 0%, rgba(254,180,19,0.4) 30%, rgba(254,180,19,0.08) 60%, rgba(254,180,19,0.4) 85%, #FEB413 100%)",
        }}
      >
        <div
          ...
          style={{
            borderRadius: "28px",
            background: "radial-gradient(ellipse at 50% 0%, rgba(254, 180, 19, 0.06) 0%, rgba(254, 180, 19, 0.00) 60%), #0D0A06",
          }}
        >
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-golos text-[11px] text-white/45">{t('marketplace.arsenalIndex')}</p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-mono text-[11px] text-[#3FC86A]/80 uppercase tracking-wider">{t('marketplace.bids')}</span>
                    <span className="font-mono text-[11px] text-white/40">{t('marketplace.size')}</span>
                  </div>
                  {ORDER_BOOK_BIDS.map((row, i) => (
                    <div key={i} className="relative flex items-center justify-between py-[3px] px-1.5 rounded-sm mb-[2px]">
                      <div className="absolute inset-0 rounded-sm bg-[#3FC86A]/8" style={{ width: `${row.fill}%` }} />
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-mono text-[11px] text-[#FF5A5A]/80 uppercase tracking-wider">{t('marketplace.asks')}</span>
                    <span className="font-mono text-[11px] text-white/40">{t('marketplace.size')}</span>
                  </div>
                  {ORDER_BOOK_ASKS.map((row, i) => (
                    <div key={i} className="relative flex items-center justify-between py-[3px] px-1.5 rounded-sm mb-[2px]">
                      <div className="absolute right-0 inset-y-0 rounded-sm bg-[#FF5A5A]/8" style={{ width: `${row.fill}%` }} />
              ...
              <div className="flex items-center justify-between pt-2 border-t border-white/5">
                <div className="flex gap-4">
                  <div>
                    <span className="font-mono text-[11px] text-white/40 block">{t('marketplace.vol24h')}</span>
                    <span className="font-mono text-xs text-white/70">$847.2K</span>
                  </div>
                  <div>
                    <span className="font-mono text-[11px] text-white/40 block">{t('marketplace.spread')}</span>
          ...
          >
            ...
            >
              {t('marketplace.getEarlyAccess')}
            </button>
            <p className="font-golos text-xs text-white/40">
              {t('marketplace.earlyAccessNote')}
            </p>
          </motion.div>
        </div>
      </div>
