...
const StatCard: React.FC<StatCardProps> = ({ value, label }) => (
  <div className="flex-1 flex flex-col items-center justify-center gap-2 px-8 py-6 rounded-xl border border-[#3f392b] bg-[#161104]/60 backdrop-blur-sm min-w-35">
    <span className="font-jura font-bold text-3xl sm:text-4xl text-white tracking-tight">{value}</span>
    <span className="font-golos text-xs sm:text-sm text-white/40 text-center leading-snug">{label}</span>
  </div>
);
...
export const WhyPolymarketSection: React.FC = () => {
  ...
  const timeline = [
    { label: t('whyPolymarket.hours4'), value: '96.7%' },
    { label: t('whyPolymarket.hours12'), value: '96.4%' },
    { label: t('whyPolymarket.day1'), value: '95.8%' },
    { label: t('whyPolymarket.week1'), value: '94%' },
    { label: t('whyPolymarket.month1'), value: '90.4%' },
  ];
    ...
    <section className="w-full py-24 px-4 sm:px-10 lg:px-30 bg-[#0D0A06]">
      <div
        ...
        style={{
          padding: "13px",
          borderRadius: "30px",
          background: "linear-gradient(to bottom, #FEB413 0%, rgba(254,180,19,0.6) 40%, rgba(254,180,19,0.1) 75%, rgba(254,180,19,0) 100%)",
        }}
      >
        <div
          ...
          style={{
            borderRadius: "17px",
            background: "radial-gradient(141.42% 70.71% at 50% 0%, rgba(254, 180, 19, 0.08) 0%, rgba(254, 180, 19, 0.00) 60%), #0D0A06",
          }}
        >
          <motion.span
          ...
          >
            <BracketSvg />
            <BracketSvg flipped />
            {t('whyPolymarket.badge')}
          </motion.span>

          <GradientHeading className="text-4xl sm:text-5xl lg:text-6xl max-w-4xl">
            {t('whyPolymarket.heading')}
          </GradientHeading>

          <div className="w-full flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center mt-2">
            <StatCard value="96.7%" label={t('whyPolymarket.accuracy4h')} />
            <StatCard value="90.4%" label={t('whyPolymarket.accuracy1m')} />
            <StatCard value="$30.5B" label={t('whyPolymarket.totalVolume')} />
          </div>

          <div className="w-full max-w-xl mx-auto mt-2">
            <p className="font-jura text-[10px] uppercase tracking-widest text-white/30 mb-3 text-center">{t('whyPolymarket.accuracyPrior')}</p>
          ...
          >
            {t('whyPolymarket.viewAccuracy')} <span aria-hidden="true">→</span>
          </a>
        </div>
      </div>
    </section>
